...
class TF(object):
    ...
    def save_object_to_file(self, file):
        ...
        for rule in self.rules:
            ...
            st = ""
            print "##Rule Match={}".format(rule["match"])
            if rule["action"]=="link":
                f.write("%s$" % "None")
                f.write("%s$" % "None")
                f.write("%s$" % "None")
                ...
                s1 = ""
                strs = wildcard_to_str(rule["match"]).split(',')
                for s in strs:
                    s1 += s
                f.write("%s$"%s1)
            ...
            f.write("$")
            f.write("#")
            print "rule={} influes_on={}/{}".format(rule["id"], rule["influence_on"], len(rule["influence_on"]))
            for io in rule["influence_on"]:
    ...
    def load_from_str(self, text):
        print "=== Loading transfer function from JSON ==="
        func = json.loads(text)
        print "=== load_from_str ## input JSON={} ===".format(func)
        self.rules = []
